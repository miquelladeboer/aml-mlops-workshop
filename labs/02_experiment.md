...
In this tuturial, we are going to track the progress of or experiments with Azure Machine learing. We are going to enhance the model creation process by tracking your experiments and monitoring run metrics. In this lab, learn how to add logging code to your training script, submit an experiment run, monitor that run, and inspect the results in Azure Machine Learning. We are going to perform the following steps in the lab:
* Understand the non Azure Machine Learning training script
* Run the train script local
* Refactor the train script to log metrics to Azure Machine Learning
* Create a submit file to submit a run as an experiment to Azure Machine Learning
* Submit an experiment with local compute and local data
* Monitor the run
